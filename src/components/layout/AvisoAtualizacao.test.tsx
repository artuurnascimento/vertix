...
const { estado } = vi.hoisted(() => ({
  estado: { nova: null as null | { versao: string; data: string; itens: string[] } },
}))
...
describe('AvisoAtualizacao', () => {
  ...
  test('mostra a faixa, abre a descrição do que mudou e recarrega ao atualizar', () => {
    estado.nova = { versao: 'b2c3d4', data: '', itens: ['Cronômetro no topo', 'Imagens otimizadas'] }
    const reload = vi.fn()
    ...
    fireEvent.click(screen.getByRole('button', { name: /O que mudou desde a sua versão/ }))
  ...
  test('adiar esconde esta versão', () => {
    estado.nova = { versao: 'b2c3d4', data: '', itens: [] }
    render(<AvisoAtualizacao />)
