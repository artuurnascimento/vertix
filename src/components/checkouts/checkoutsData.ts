...
export interface Checkout {
  ...
  banner: unknown
  /**
   * Imagem do order bump, mesma forma do `banner` (desktop, mobile, alt) e
   * mesmo parser. `{}` = bump sem imagem.
   */
  bump_imagem: unknown
  ...
  /**
   * Cronômetro por visitante, em minutos: cada abertura da página começa com
   * N minutos e, ao zerar, fica em 00:00 até sair. null = não usa este
...
export type CheckoutPayload = Omit<
  Checkout,
  'id' | 'created_at' | 'updated_at' | 'prova' | 'banner' | 'bump_imagem'
