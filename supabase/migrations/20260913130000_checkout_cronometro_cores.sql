-- ---------------------------------------------------------------------------
-- Cores da faixa do cronômetro
-- ---------------------------------------------------------------------------
-- A faixa do cronômetro na página pública nasce no roxo da Vertix com texto
-- branco. Quem monta a oferta precisa poder trocar as duas cores (uma
-- oferta de Black Friday preta e amarela, por exemplo) sem mexer em código.
--
-- `cronometro_cor_fundo` e `cronometro_cor_texto`: hex de 6 dígitos com
-- `#`, ou null = cor padrão da página. A `get_checkout_info` é republicada
-- com os dois campos dentro de `checkout` e na raiz, como os demais.
-- Aditiva: `if not exists`, nenhuma coluna existente é tocada.
-- ---------------------------------------------------------------------------

alter table public.checkouts
  add column if not exists cronometro_cor_fundo text,
  add column if not exists cronometro_cor_texto text;

do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'checkouts_cronometro_cores_hex'
  ) then
    alter table public.checkouts
      add constraint checkouts_cronometro_cores_hex
      check (
        (cronometro_cor_fundo is null or cronometro_cor_fundo ~ '^#[0-9a-fA-F]{6}$')
        and (cronometro_cor_texto is null or cronometro_cor_texto ~ '^#[0-9a-fA-F]{6}$')
      );
  end if;
end $$;

comment on column public.checkouts.cronometro_cor_fundo is
  'Cor de fundo da faixa do cronômetro (#rrggbb). null = roxo da Vertix.';
comment on column public.checkouts.cronometro_cor_texto is
  'Cor do texto da faixa do cronômetro (#rrggbb). null = branco.';

-- ---------------------------------------------------------------------------
-- get_checkout_info — republicada com as cores
-- ---------------------------------------------------------------------------
-- Cópia literal da definição de 20260913120000 com DUAS adições em dois
